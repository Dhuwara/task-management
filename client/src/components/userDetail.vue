<template>
     <div class="main-con" style="height: 90vh;">
  ...
  <div class="d-flex justify-content-between">
    ...
    <div class="Edit-btn">
      <button class="btn">
        <font-awesome-icon :icon="['fas', 'pen-to-square']" style="color: #b3b9cb;" />
      </button>
    </div>
  </div>
  
  <!-- Assuming user.address contains the full address -->
  <div class="d-flex justify-content-between me-3" v-if="user.address">
    <div class="d-flex flex-column">
      <p style="font-size: small; color: black">Country</p>
      <p style="font-size: small; color: #AFB6C8; font-weight: 600;">
        {{ splitAddress(user.address).country }}
      </p>
    </div>
    <div class="d-flex flex-column">
      <p style="font-size: small; color:black">City/State</p>
      <p style="font-size: small; color: #AFB6C8; font-weight: 600;">
        {{ splitAddress(user.address).city }}, {{ splitAddress(user.address).state }}
      </p>
    </div>
  </div>
...
export default {
  ...
  try {
    ...
    const taskIds = Array.isArray(this.user.task_ids) ? this.user.task_ids : [this.user.task_ids];
    ...
    this.user.tasks = await Promise.all(taskPromises);
  } catch (error) {
...
splitAddress(address) {
    ...
    // Assuming the address format is "Street, City, State, Country"
    ...
    return { street: '', city: '', state: '', country: '' }; // Return empty if format doesn't match
  }
